...
export default function FindInvestment({ onSave }) {
  ...
  const addItem = (item) => {
    const isAdded = addedItems.some(
      (addedItem) => item.srtnCd === addedItem.srtnCd
    );
        ...
        <div className="input-group">
          <label>
            ...
            <input
              ...
              placeholder="예) 원전주 모음"
            />
          </label>
          <button className="action-button" onClick={handleSetList}>
            리스트로 저장
          </button>
        </div>
        <div className="input-group">
          ...
          <button className="action-button" onClick={findStock}>
            검색
          </button>
        </div>
      </div>
